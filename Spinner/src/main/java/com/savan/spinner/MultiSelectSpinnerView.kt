 ...
 */
open class MultiSelectSpinnerView(context: Context, attrs: AttributeSet?) :
    ...
    private var selectedDropDownArrayList: HashSet<String> = HashSet()
    ...
    fun buildCheckedSpinner(
        ...
        selectedSectionListener: (ArrayList<Int>, HashSet<String>) -> Unit
    ) {
...
//            returnSectionColl = returnSectionColl.replace(", $".toRegex(), "")
            ...
            if (returnSectionColl.isEmpty()) {
                selectedSectionListener(selectedPosition, HashSet())
            } else {
                for (i in spinnerModelList) {
                    if (i.isSelected) {
                        selectedDropDownArrayList.add(i.text)
                        if (i.text == "All"){
                            selectedDropDownArrayList.remove(i.text)
                        } else if (i.text == "OK"){
                            selectedDropDownArrayList.remove(i.text)
                        }
                    } else {
                ...
                selectedSectionListener(selectedPosition, selectedDropDownArrayList)
            }
            ...
            this.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(
                    ...
                    text?.setTextColor(Color.parseColor("#D7100F0F"))
